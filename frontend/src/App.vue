...
<template>
    ...
    <!-- ===================== AUTH VIEW ===================== -->
    <div v-if="view === 'auth'" class="min-h-screen bg-surface-container-high flex items-center justify-center p-4 md:p-10">

        <!-- LOGIN TAB -->
        <div v-if="authTab === 'login'" class="w-full max-w-md bg-canvas-white rounded-lg border border-hairline p-6 md:p-8">
            <!-- Logo -->
            ...
            <form @submit.prevent="login" class="flex flex-col gap-6">
                <div class="flex flex-col gap-2">
                    <label class="font-mono text-label-mono uppercase text-on-surface text-xs tracking-widest" for="login-nim">NIM</label>
                    <input
                ...
                <div class="flex flex-col gap-2">
                    <label class="font-mono text-label-mono uppercase text-on-surface text-xs tracking-widest" for="login-password">Password</label>
                    <input
            ...
            <div class="mt-6 text-center border-t border-hairline pt-6">
                <p class="text-slate text-sm">
                    Need an account?
                    <button @click="authTab = 'register'" class="text-ink hover:text-action-blue underline transition-colors ml-1">Register here</button>
                </p>
            </div>
        </div>
        ...
        <div v-else class="w-full max-w-md px-0">
            ...
            <div class="bg-canvas-white rounded-lg border border-hairline p-8">
                ...
                <form @submit.prevent="register" class="space-y-4">
                    <div>
                        <label class="block font-mono text-xs uppercase text-primary tracking-widest mb-2" for="reg-name">Full Name</label>
                        <input v-model="registerForm.name" id="reg-name" type="text" required placeholder="Jane Doe" class="input-field" />
                    </div>
                    <div>
                        <label class="block font-mono text-xs uppercase text-primary tracking-widest mb-2" for="reg-nim">NIM</label>
                        <input v-model="registerForm.nim" id="reg-nim" type="text" required placeholder="2451xxxxxx" class="input-field" />
                    </div>
                    <div>
                        <label class="block font-mono text-xs uppercase text-primary tracking-widest mb-2" for="reg-email">Email</label>
                        <input v-model="registerForm.email" id="reg-email" type="email" placeholder="johndoe@example.com" class="input-field" />
                    </div>
                    <div>
                        <label class="block font-mono text-xs uppercase text-primary tracking-widest mb-2" for="reg-password">Password</label>
                        <input v-model="registerForm.password" id="reg-password" type="password" required placeholder="••••••••" class="input-field" />
                    </div>
                    <div class="pt-2">
                ...
                <div class="mt-6 text-center text-sm text-slate">
                    ...
                    <button @click="authTab = 'login'" class="text-ink underline hover:text-action-blue ml-1 transition-colors">Log in here</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div v-if="view === 'dashboard'" class="min-h-screen bg-background font-body-md text-on-background flex">
        ...
        <!-- Sidebar (Desktop) -->
        <aside class="hidden md:flex flex-col bg-surface-container text-on-surface h-full w-64 border-r border-hairline fixed left-0 top-0 pt-20 pb-8 px-4 z-40">
            <div class="mb-6 px-4">
            ...
            <nav class="flex-1 flex flex-col gap-1">
                <button
                    @click="uploadView = false"
                    :class="!uploadView ? 'nav-item-active' : 'nav-item'"
            ...
            <div class="mt-auto flex flex-col gap-1 pt-4 border-t border-hairline">
                ...
                <button
                    @click="logout"
                    class="nav-item text-left w-full text-error-red hover:bg-red-50"
                >
        ...
        <!-- Mobile Top Bar -->
        <div class="md:hidden fixed top-0 left-0 right-0 z-40 bg-canvas-white border-b border-hairline px-4 py-3 flex items-center justify-between">
            <div>
            ...
            <button @click="logout" class="text-xs text-slate hover:text-error-red transition-colors flex items-center gap-1">
        ...
        <div class="flex-1 md:ml-64 flex flex-col min-h-screen w-full">
            ...
            <!-- ===== LIBRARY VIEW ===== -->
            <main v-if="!uploadView" class="flex-1 bg-canvas-white pt-20 md:pt-24 pb-16 px-4 md:px-10 max-w-[1200px] mx-auto w-full">
                <!-- Page Header -->
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-hairline pb-6 gap-4">
                    <div>
                ...
                <div v-if="files.length === 0" class="flex flex-col items-center justify-center py-24 text-center">
                    <span class="material-symbols-outlined text-muted-slate mb-4" style="font-size:48px;">photo_library</span>
                    ...
                    >
                        ...
                        <div class="h-48 w-full bg-surface-container-low relative overflow-hidden">
                            <img
                                ...
                                :src="`/api/files/${file.original_name}`"
                                :alt="file.original_name"
                                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                            />
                            <div v-else class="w-full h-full flex flex-col items-center justify-center text-muted-slate">
                                <span class="material-symbols-outlined" style="font-size:40px;">description</span>
                                <span class="text-xs mt-2 font-mono uppercase">{{ getFileExt(file.original_name) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="p-4 flex flex-col gap-2 border-t border-hairline bg-surface-container-low">
                            <div class="flex justify-between items-start">
                                <h3 class="text-sm font-semibold text-primary truncate pr-2">{{ file.original_name }}</h3>
                                <span class="font-mono text-xs uppercase text-slate shrink-0 bg-surface-container px-2 py-0.5 rounded">{{ getFileExt(file.original_name) }}</span>
                            </div>
            ...
            <main v-if="uploadView" class="flex-1 flex flex-col min-h-screen">
                ...
                <div class="bg-secondary text-on-secondary pt-20 md:pt-24 pb-6 px-4 md:px-10 border-b border-hairline">
                    <div class="max-w-[1200px] mx-auto w-full flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h1 class="text-2xl md:text-3xl font-semibold text-on-secondary tracking-tight">Media Upload</h1>
                            <p class="text-secondary-fixed-dim text-sm mt-1">Add new assets to your operational library.</p>
                        </div>
                        <button @click="uploadView = false" class="flex items-center gap-2 text-secondary-fixed-dim hover:text-on-secondary text-sm transition-colors">
                ...
                <div class="flex-1 w-full max-w-[1200px] mx-auto p-4 md:p-10 bg-surface">
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div class="lg:col-span-5 flex flex-col gap-6">
                            <div class="bg-canvas-white p-6 rounded-lg border border-hairline flex flex-col gap-5">
                                ...
                                <form @submit.prevent="uploadFile" class="flex flex-col gap-5">
                                    <!-- File Picker -->
                                    <div class="flex flex-col gap-2">
                                        <label class="font-mono text-xs uppercase text-slate tracking-widest">Select File</label>
                                        <div
                                            class="border-2 border-dashed rounded-lg p-8 flex flex-col items-center justify-center gap-3 transition-colors cursor-pointer"
                                            ...
                                            @drop.prevent="onDrop"
                                        >
                                            <span class="material-symbols-outlined text-muted-slate" style="font-size:36px;">upload_file</span>
                                            <p class="text-sm text-slate text-center">
                                                <span v-if="selectedFile" class="text-primary font-medium">{{ selectedFile.name }}</span>
                                                <span v-else>Click to choose a file or drag & drop</span>
                                            </p>
                                            <p v-if="selectedFile" class="text-xs text-muted-slate">{{ (selectedFile.size / 1024 / 1024).toFixed(2) }} MB</p>
                                        </div>
                                        <input
                                            ref="fileInput"
                                            type="file"
                                    ...
                                    >
                                        <span v-if="loading" class="material-symbols-outlined animate-spin" style="font-size:16px;">progress_activity</span>
                                        <span v-else class="material-symbols-outlined" style="font-size:16px;">cloud_upload</span>
                        ...
                        <div class="lg:col-span-7 flex flex-col">
                            <div class="bg-canvas-white rounded-media border border-hairline overflow-hidden flex flex-col h-full sticky top-24">
                                <!-- Preview header -->
                                <div class="bg-surface-container-low flex items-center gap-2 p-4 border-b border-hairline">
                                    <span class="material-symbols-outlined text-slate" style="font-size:18px;">visibility</span>
                                    <span class="font-mono text-xs text-slate uppercase tracking-widest">Preview Source</span>
                                </div>

                                <!-- Preview area -->
                                <div class="flex-1 bg-surface-container-lowest p-4 relative flex items-center justify-center min-h-[300px] md:min-h-[400px]">
                                    <!-- Checkered bg -->
                                    ...
                                    <template v-else-if="selectedFile">
                                        <div class="relative z-10 flex flex-col items-center gap-3 text-slate">
                                            <span class="material-symbols-outlined" style="font-size:56px;">description</span>
                                            <p class="text-sm font-medium text-primary">{{ selectedFile.name }}</p>
                                            <p class="font-mono text-xs uppercase text-slate">{{ getFileExt(selectedFile.name) }} · {{ (selectedFile.size / 1024 / 1024).toFixed(2) }} MB</p>
                                        </div>
                                    </template>
                                    <template v-else>
                                        <div class="relative z-10 flex flex-col items-center gap-3 text-muted-slate">
                                            <span class="material-symbols-outlined" style="font-size:56px;">image_search</span>
